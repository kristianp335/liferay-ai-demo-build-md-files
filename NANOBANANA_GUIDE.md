...
- **Size Specification**: Use the `sizes` parameter to specify desired pixel dimensions.
  - Example: `/icon prompt="blue circle icon" sizes=[16, 32, 64]`
...
- **Size Specification**: Use the `size` parameter to specify pattern tile dimensions as a string.
  - Example: `/pattern prompt="geometric blue pattern" size="256x256"`
...
- **Resolution and Aspect Ratios**: Ensure appropriate resolution and aspect ratios. **Important Note:** Through testing, it has been observed that the `/generate` command currently produces images with a fixed resolution of 1024x1024 pixels, regardless of attempts to specify size via the `--size` parameter or within the prompt text. **For Liferay fragments or specific dimension requirements, users have two primary options for image sourcing: either generate images at this default size using `nanobanana` and then use external tools like Python's Pillow library for resizing or cropping, or obtain images from reference sites using tools like `web_fetch` or Playwright for download.**


### Image Cropping with Python (Pillow)

Since the `/generate` command produces square images 1000x1000 pixels by default, you will often need to crop them for specific uses, especially within Liferay fragments that require particular aspect ratios (e.g., hero banners, card images). Python's Pillow library is recommended for this task.
...
# --- Usage Example (assuming a_monkey.png exists) ---
# input_image = 'nanobanana-output/a_monkey.png'
# output_dir = 'nanobanana-output'
# os.makedirs(output_dir, exist_ok=True)
# crop_to_size(input_image, 300, 500, os.path.join(output_dir, 'a_monkey_cropped_300x500.png'))
```

**2. Cropping to a Specific Aspect Ratio (e.g., 16:9 Landscape)**

This script calculates the largest possible rectangle with the target aspect ratio that fits within the original image, and then crops that centered rectangle.
...
def crop_to_aspect_ratio(image_path, target_aspect_ratio, output_path):
    with Image.open(image_path) as img:
        ...
        # Calculate the dimensions of the crop box that fits within the original image
        # while maintaining the target_aspect_ratio.
        
        # Determine crop dimensions based on which original dimension is limiting
...
# output_dir = 'nanobanana-output'
# os.makedirs(output_dir, exist_ok=True)
# crop_to_aspect_ratio(input_image, 16 / 9, os.path.join(output_dir, 'a_monkey_landscape_16_9.png'))
```
